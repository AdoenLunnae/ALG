...
long double Algorithm::calculateISEValue(const int& initial, const int& final)
{
    ...
    if (fabs(B) > 0.0000000001) //line not  parallel to y axis
    {
        a = -C / B;
        b = -A / B;
        ISE = ((final - initial - 1.0) * a * a + 2.0 * a * b * _sX.getElement(initial, final) + b * b * _sXX.getElement(initial, final) - 2.0 * a * _sY.getElement(initial, final) - 2.0 * b * _sXY.getElement(initial, final) + _sYY.getElement(initial, final)) / (b * b + 1.0);

    } else //Line parallel to y axis. In this case b is infinite and the error is calculated in another way
    {
        ISE = _sXX.getElement(initial, final) + (2.0 * (C / A)) * _sX.getElement(initial, final) + (final - initial - 1) * (C / A) * (C / A);
    }
